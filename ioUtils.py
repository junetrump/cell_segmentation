...
class Batch_Feeder:
    ...
    def set_paths(self, idList=None, imageDir=None, gtDir=None, ssDir=None):
        ...
        if self._train:
            for id in idList:
                self._paths.append([id, imageDir + '/' + id + '.tif',
                                        gtDir + '/' + id + '.mat',
                                        ssDir + '/' + id + '.mat'])
            self.shuffle()
        else:
        ...
        #                             os.path.join(imageDir, id + '.tif'),
        #                             os.path.join(gtDir, id + '.mat'),
        #                             os.path.join(ssDir, id + '.mat')])
        #     self.shuffle()
        # else:
        #     for id in idList:
        #         image_path = os.path.join(imageDir, "{}.tif".format(id))
        #         ss_path = os.path.join(ssDir, "{}.mat".format(id))
        #         self._paths.append([id, image_path, ss_path])
        #
        # self._numData = len(self._paths)
    ...
    def next_batch(self):
        ...
        if self._train:
            while(len(idBatch) < self._batchSize):
                ...
                # ss = (sio.loadmat(r'' + self._paths[self._index_in_epoch][3])['mask']).astype(float)
                # ssMask = np.where(ss == 1, 1, 0).astype(float)
                ...
                # ssMask = np.concatenate((ssMask, background), axis=-1)
                # ssMask = np.argmax(ssMask, axis=-1)
                # ssMask = ssMask.astype(float)
                # ssMask = (ssMask > 0).astype(float) # centered at 0, with 0 being background, spaced 32 apart for classes
            ...
            for example in self._paths[self._index_in_epoch:min(self._index_in_epoch+self._batchSize, self._numData)]:
                ...
                # ss = skimage.io.imread(example[2])

                # ssMask = np.where(ss == 1, 1, 0).astype(float)
                # img = Image.open(example[2])
                # ss = np.array(img).astype(float)
